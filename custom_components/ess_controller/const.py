...
def get_default_activation_time():
    """Get the default activation time."""
...
INFLUX_UPDATE_INTERVAL = timedelta(minutes=30)  # InfluxDB update interval
TARGET_SOC_UPDATE_INTERVAL = timedelta(minutes=10)  # Target SoC update interval
...
def create_step_one_schema(config_entry=None):
    return vol.Schema({
        vol.Required("activation_time", default=get_existing_or_default(config_entry, "activation_time", get_default_activation_time)): str,
        vol.Required("time_zone", default=get_existing_or_default(config_entry, "time_zone", DEFAULT_TIME_ZONE)): vol.In(pytz.all_timezones),
...
def create_step_two_schema(config_entry=None):
    return vol.Schema({
        vol.Required("pvpc_buy_entity", default=get_existing_or_default(config_entry, "pvpc_buy_entity", DEFAULT_PVPC_BUY_ENTITY)): str,
        vol.Required("pvpc_sell_entity", default=get_existing_or_default(config_entry, "pvpc_sell_entity", DEFAULT_PVPC_SELL_ENTITY)): str,
...
def create_step_three_schema(config_entry=None):
    return vol.Schema({
        vol.Required("acin_to_acout_sensor", default=get_existing_or_default(config_entry, "acin_to_acout_sensor", DEFAULT_ACIN_TO_ACOUT_SENSOR)): str,
        vol.Required("inverter_to_acout_sensor", default=get_existing_or_default(config_entry, "inverter_to_acout_sensor", DEFAULT_INVERTER_TO_ACOUT_SENSOR)): str,
        vol.Required("battery_soc_sensor", default=get_existing_or_default(config_entry, "battery_soc_sensor", DEFAULT_BATTERY_SOC_SENSOR)): str,
        ...
        vol.Required("solar_production_sensor", default=get_existing_or_default(config_entry, "solar_production_sensor", DEFAULT_SOLAR_PRODUCTION_SENSOR)): str,

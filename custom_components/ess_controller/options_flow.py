...
from .const import create_step_one_schema, create_step_two_schema, create_step_three_schema, create_step_four_schema
...
class PVcontrollerOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_three(self, user_input=None):
        ...
        if user_input is not None:
            try:
                ...
                # Store the data from the second step and proceed to the third step
                self.context["step_three_data"] = user_input
    ...
    async def async_step_four(self, user_input=None):
        """Handle the fourth step of the options flow."""
        errors = {}
        if user_input is not None:
            try:
                # Validate user input for the third step
                create_step_four_schema(self.config_entry)(user_input)
                # Combine the data from all steps
                data = {**self.context["step_one_data"], \
                        **self.context["step_two_data"], \
                        **self.context["step_three_data"], \
                        **user_input}
        ...
        return self.async_show_form(
            step_id="four",
            data_schema=create_step_four_schema(self.config_entry),
            errors=errors
        )

...
from .utils import get_entity_description, get_coordinator_from_entity_name
...
async def async_setup_entry(hass, config_entry, async_add_entities):
    """Set up the pv_controller sensor based on config entry."""
    time_zone = config_entry.data.get("time_zone")
    # Add ClockSensor
    name = config_entry.title + " Clock"
    async_add_entities([ClockSensor(hass, name, time_zone, config_entry)])
...
class ClockSensor(SensorEntity):
    """
    Clock sensor. Example class not linked to the coordinator, governed by async_track_time_interval.
    """
    
    def __init__(self, hass: HomeAssistant, name, time_zone, config_entry):
        """Initialize the clock sensor."""
        self._attr_name = name or "pv_controller_clock"
        self._time_zone = time_zone or config_entry.options.get("time_zone", "UTC")  # Get the updated time_zone
        now = self._get_current_time_in_timezone()
        current_time = now.strftime("%H:%M")        
        self._state = current_time
        self._config_entry = config_entry
        self._attr_should_poll = False  # Disable polling, we will update manually
        
        self._hass = hass
        # Configure the sensor update every 1 second
        async_track_time_interval(self._hass, self._async_update_sensor, timedelta(seconds=1))        
    ...
    def icon(self):
        ...
        # You can return the name of a Material Design Icons (MDI) icon
    ...
    async def _async_update_sensor(self, now):
        """Fetch new state data for the sensor."""
        # Get the updated options from the config_entry
        self._time_zone = self._config_entry.options.get("time_zone", "UTC")
                    
        try:            
            # now = self._get_current_time_in_timezone()
            now = await asyncio.to_thread(self._get_current_time_in_timezone)
            self._state = now.strftime("%H:%M")           
                      
        except pytz.UnknownTimeZoneError:
            self._state = "Invalid Timezone"
        
        self.async_write_ha_state()  # Notify Home Assistant that the state has changed
        
    def _get_current_time_in_timezone(self):
        """Get the current time in the specified time zone (executed outside the event loop)."""
        timestamp = time.time()
        now = datetime.fromtimestamp(timestamp, ZoneInfo(self._time_zone))
        return now
